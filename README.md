...
- Saved Profile switching is wired from the UI to a combined backend transaction. With explicit user confirmation it closes running Desktop/VS Code processes, restores selected environments, restarts supported apps, records history, and rolls back if restore or restart fails. The switch dialog displays transaction phase outcomes, identity verification status, and the non-secret transaction event timeline returned by the backend.
...
- Manual restart retry buttons are available for Codex Desktop and VS Code from the switch result dialog and Environment page. They reuse the same platform restart controllers and report errors without modifying Profile state.
