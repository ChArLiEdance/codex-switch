...
- Current environment import is implemented for selected CLI, VS Code, and Desktop detector results. The Profiles UI now guides the official-login-first workflow, shows read-only current account evidence beside the import controls, previews capture coverage and skipped reasons before saving, and blocks multi-environment imports until same-account confirmation is checked.
...
- Successful restore transactions update Profile `lastUsedAt`, record the previous Profile id and display name in local history, and expose dedicated Home actions for restoring the default Profile or switching back to the previous Profile without relying on display-name uniqueness. These actions return explicit skipped/attempted reasons from the backend.
- The Usage page provides a local-only view of Codex CLI activity. It scans `~/.codex/sessions` and `~/.codex/archived_sessions` for JSONL `token_count` events, summarizes input/cached/output tokens, displays recent session history, and surfaces rate-limit remaining percentages when Codex writes them. It does not read or display prompt text or response content.
- Recovery detection surfaces unfinished transaction journals, matching backup manifest availability, backup entry counts, and the latest transaction event. When rollback evidence is available, the Home view can run a manual recovery rollback from the persisted backup manifest; otherwise the user can mark a reviewed journal failed and use Restore default or Switch back for an explicit corrective switch.
