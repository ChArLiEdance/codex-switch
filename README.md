...
- Recovery detection surfaces unfinished transaction journals, matching backup manifest availability, backup entry counts, and the latest transaction event before letting the user mark a reviewed journal failed and use Restore default or Switch back for an explicit corrective switch.
