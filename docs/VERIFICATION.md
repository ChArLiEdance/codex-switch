...
- `cargo test` succeeds with tests covering profile metadata, profile update/delete/last-used behavior, secret-store abstraction, typed VS Code/Desktop detector candidates, settings-managed custom detector paths, redacted account-hint parsing, redacted environment diagnostics, import preflight coverage, import, transaction backup/rollback/journaling, timestamped backup manifest persistence, backup-manifest-aware recovery status, manual recovery rollback from persisted backup manifests, duplicate target, direct symlink target, nearest existing symlink target ancestor, existing non-file target, unsafe transaction ID, symlink backup root, non-directory backup root rejection before backup or writes, backup byte readback verification before restore, transactional cache refresh, restored auth/config byte and Unix-mode readback verification, rollback result readback verification, terminal transaction failure reporting when rollback verification fails, Desktop coordinator, CLI coordinator including active-task process filtering, VS Code coordinator, post-restart process verification and rollback, switch identity verification, manual restart retry, settings, history with stable profile ids and legacy history compatibility, manual restore-default and switch-previous recovery commands, read-only Codex session usage parsing, restore-default-on-exit, and recovery detection/resolution state.
...
- `npm run build` type-checks Home recovery rollback controls and recovery backup evidence rendering.
- `npm run build` type-checks Home Restore default and Switch back actions using dedicated backend recovery commands and id-aware switch history.
- `npm run build` type-checks the Usage page, local token totals, quota window rendering, session history rows, and switch-history reuse.
...
- `npm run build` type-checks the guided official-login import controls, current-state evidence display, frontend same-account import preflight, and import capture coverage preview.
- `npm run build` type-checks the Environment page diagnostics report generation, display, and clipboard action.
