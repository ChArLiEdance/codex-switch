...
- Import capture coverage preview with skipped-reason summaries before saving a Profile
...
- Terminal transaction failure events when rollback verification itself fails
...
- Dedicated Home recovery commands for restoring the default Profile and switching back to the previous Profile, including backend skipped/attempted reasons
- Usage page for read-only Codex JSONL session token summaries, quota windows, recent local session history, and switch history
