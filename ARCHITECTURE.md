...
7. Runs one `TransactionRunner` target-validation/backup/restore/cache-refresh/readback-verification/rollback transaction.
...
10. Rolls back restored files and refreshed cache paths if restore, cache refresh, restored-file readback verification, post-restore restart, or post-restart process verification fails.
...
The Home view derives the current Profile from the latest `lastUsedAt` value, then offers quick actions to restore the default Profile or switch back to the previous Profile recorded in history. `restore_default_profile` and `switch_previous_profile` are UI-facing recovery commands with explicit `attempted`, `reason`, `targetProfile`, `environments`, and nested `switchResult` fields. New history entries store stable `fromProfileId` / `toProfileId` values so the Switch back target is not confused by duplicate display names; legacy history without ids falls back to the old name lookup and reports ambiguity instead of guessing. Both commands reuse the normal `switch_to_profile` transaction path and the configured default switch scope.
